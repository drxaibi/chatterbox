...
$subtitle = 'Convert source speech into target voice.'
...
& '.\.venv\Scripts\python.exe' '.\gradio_vc_app.py'
...
$helperScriptPath = Join-Path ([System.IO.Path]::GetTempPath()) ("chatterbox-start-vc-{0}.ps1" -f ([guid]::NewGuid().ToString('N')))
